...
11. Базовые доктрины MVP: `Strike`, `Hack`, `Virus`, `Phantom`, `Shield`, `Overload`, `Trap`. Это единственный целевой набор доктрин.
...
сохраняет выбранную human army;
возвращает player join state.
```
...
проверяет, что игрок уже выбрал army;
проверяет, что battle активна;
проверяет, что игрок ещё не делал приказ в этой battle;
сохраняет hidden doctrine order в Redis;
возвращает OrderResponse и обновленный PlayerBattleState.
```

#### POST `/api/spy/respond`
...
20–30 сек:
Показываем боевой экран с 7 доктринами: Strike / Hack / Virus / Phantom / Shield / Overload / Trap + поддействие.
...
```txt
первый экран = карта + таймер + 7 доктрин с короткими counter-подсказками;
...
7 доктрин как основная механика;
приказ игрока;
таймер 3 минуты;
ручной resolve;
простые результаты.
...
Каждая фаза длится 3 минуты в демо. В конце фазы две человеческие дивизии и ИИ сталкиваются за территорию. Победитель определяется через 7-доктринную систему `Strike / Hack / Virus / Phantom / Shield / Overload / Trap`, координацию, разведку, шпионское влияние, эффект территории и публичный шум комментариев. Территория меняет цвет или остаётся спорной. ИИ публикует язвительный комментарий. Игрок получает XP, звание, медали и причину вернуться в следующую фазу.
